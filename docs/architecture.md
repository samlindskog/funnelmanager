...
**Agents** — a client-credentials client (see `agent-example`) gets an
`aud: mcp` token and calls `/mcp` (workload-gated by OPA `callers.mcp`,
empty until an agent runner exists). The MCP server exchanges the token
toward leads (`azp: mcp`); the agent's *service account* must hold a
granted realm role — same rules as humans, by decision.
...
- Static SPA images still run stock root nginx (follow-up:
  ...
  (PLACEHOLDERS).
- Mail has no per-user mailbox ownership (Phase 0 decision): any principal
  whose role covers `mail` sees all mailboxes.
