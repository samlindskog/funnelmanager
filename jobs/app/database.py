...
async def _create_database_if_missing() -> None:
    ...
    existing deployment would never grow the jobs database — create it from
    here against the admin database instead (CREATE DATABASE cannot run in a
    transaction, hence AUTOCOMMIT). Mirrors the mail service.
    """
...
async def init_db() -> None:
    ...
    except Exception:  # noqa: BLE001 — any create_all failure retries once via DB-create
