...
/** The Prospect workflow: paste N company domains → resolve each to an Apollo org
 * → (optionally skip already-ingested) → people-ingest each → hand the resolved
 * company set to a saved-leads similarity search. This hook owns the state
 * machine, the bounded-concurrency queue, and a localStorage checkpoint so a
 * refresh/crash resumes mid-run instead of re-entering everything. It is
 * UI-only orchestration over existing search endpoints — no new backend surface. */

export type ProspectPhase = 'idle' | 'resolving' | 'confirm' | 'ingesting' | 'done'
...
function downgradeTransient(row: ProspectRow): ProspectRow {
...
export interface UseProspectRun {
  phase: ProspectPhase
  rows: ProspectRow[]
  domainCount: number
  skipIfIngested: boolean
...
export function useProspectRun(): UseProspectRun {
  const progress = useProgress()
  const [rows, setRows] = useState<ProspectRow[]>([])
  const [phase, setPhase] = useState<ProspectPhase>('idle')
  ...
  const rowsRef = useRef<ProspectRow[]>(rows)
  ...
  const patchRow = useCallback(
    (
      domain: string,
      patch: Partial<ProspectRow> | ((row: ProspectRow) => Partial<ProspectRow>),
    ) => {
      setRows((prev) => {
  ...
  }, [phase, rows, skipIfIngested, clearCheckpoint])

  const setDomains = useCallback((raw: string[]) => {
    const seen = new Set<string>()
    const next: ProspectRow[] = []
  ...
  const summary = useMemo<ProspectSummary>(() => {
    let resolved = 0
    let ingested = 0
    let skipped = 0
    let notFound = 0
