...
### Roadmap / target-state appendix *(document only — do NOT implement or spawn work)*

This appendix records the intended future so the principles above are written to **accommodate** it. It is **not** a work order; no agent should begin it from this document.

**Target: a CD pipeline enabling CANARY deployments of telemetry-enabled SPAs that an AI agent can control to conduct E2E testing, with application logging folded into the loop.**

Composed of pieces the platform already has or half-has:

1. **Telemetry-enabled SPAs.** Add OpenTelemetry browser instrumentation to `frontend`/`mailui`/`agentsui` as a small, version-pinned, per-app bootstrap (browser OTel is experimental — isolate it; the SPAs share no code, so copy the ~30-line bootstrap like `oidc.ts` is copied). Outbound `/api/*` fetches originate a W3C `traceparent`; `fm_runtime` already ingests and propagates it across exchange hops. Export OTLP to a collector fanning out to Tempo (new, next to existing Prometheus/Loki), Prometheus, Loki. Emit `trace_id`/`span_id` in structured logs and Prometheus **exemplars** so metric spike → trace → log line pivots work. *(Written to satisfy P6/P10: observability is cross-cutting, owned by `fm_runtime` + the mesh, out of business code.)*

2. **Progressive delivery (canary).** Adopt **Flagger** alongside the existing Flux GitOps: per-Deployment `Canary` CRDs gating on the `fm_http_*` success-rate + P99 latency series already emitted, with automatic rollback. Gate the NDJSON streaming routes on latency/traffic-mirroring rather than naive success-rate (respecting P8). Canary policy stays declarative/versioned in git.

3. **Agent-driven E2E gate.** Flagger's confirm-promotion / pre-rollout **webhook** triggers an `agents`-service verification run that drives the canary **exclusively through MCP tools** (P3 attribution, each run a P2/`jobs` job) and queries Loki/Prometheus/Tempo for the canary's error rate/latency, returning pass/fail to gate promotion. Playwright MCP in snapshot mode authors/self-heals the browser journeys with deterministic assertions.

4. **Safety.** Verification runs obey **P4** (estimate-first; escalate over-threshold to a human even under CD automation) and carry a bounded, read-mostly tool profile + per-run cost budget (P4 research item).

The principle set is deliberately written so that **P3 (agent identity), P4 (guarded expensive actions), P6/P10 (cross-cutting observability lives in the mesh + `fm_runtime`), P8 (streaming under canary), and P11 (agent-drivable E2E)** all already point at this target — the roadmap needs no principle it doesn't have.
