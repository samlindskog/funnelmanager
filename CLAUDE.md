...
**Naming convention:** one short name per backend is used **everywhere** — source dir, compose service, container/DNS name, GHCR image (`.../{name}`), and API prefix all match: `search` (`/api/search`), `leads` (`/api/leads`), `mail` (`/api/mail`), `mcp` (`/mcp` — the MCP protocol mount, no `/api`, never nginx-routed), `jobs` (`:8005`, internal), `agents` (`:8006`, `/api/agents`), `agentsui` (`/agents/`). Every public API path is `/api/{service}`. Dev containers are `funnelmanager-{service}-1`.

**Agent-team architecture (jobs, agents, campaigns).** Beyond the original stack, the platform runs **runtime AI agents** that complete a user's task by driving the product's own APIs. Four load-bearing rules govern the whole program (full spec: `docs/agent-build-plan.md`):

- **Principle 1 — uniform functionality / cross-user visibility (within a service, gated per service).** Access is gated **per service** by a distinct realm role: `search-access`, `mail-access`, `jobs-access`, `agents-access` (plus `admin` = all, `internal-service` = leads-only, and the machine-only `jobs-internal` = the `jobs` service account's grant to reach producers' `/internal/jobs/*` stream + control endpoints on `search`/`agents`, never a human role). A principal may call a service's API **only if it holds that service's access role** — otherwise OPA (mesh) / `fm_runtime` grants (compose) return 403. *Within* a service every principal that has access sees the *same* features and the *same* data; there is **no per-user data hiding in app code** — the realm role is the only access gate, and it gates *whether you may call the API*, not *which rows* you see. So search history, mail inboxes, and campaigns are visible to **every user with that service's access role** (not to users lacking it). Writes are still *attributed* to the acting principal (`username` owner) — just not *hidden*. New read APIs browse by any user or all together. These roles are encoded in three places that must stay in sync: `fm_runtime/grants.py` `_DEFAULT_ROLE_GRANTS`, `deploy/policy/data.json` `funnelmanager.roles`, and the Keycloak realm roles (`admin` is a composite of the four `-access` roles). `python -m fm_runtime.export --check deploy/policy/data.json` machine-proves the first two equal (and `--realm` catches svc-scope over-grants); with `--realm` `verify_policy` **now also asserts the realm-role leg** — that the realm defines every role in `_DEFAULT_ROLE_GRANTS` and that `admin` is a composite of exactly the `-access` roles — so all three legs are provably in lockstep, not kept by hand.
- **`fm_origin` agent-identity model.** A runtime AI agent (the `agents` service, a confidential Keycloak client) acts via **RFC 8693 token exchange that keeps the human as the subject** (`preferred_username` unchanged = the record owner). A propagated **`fm_origin` claim** distinguishes agent-initiated calls (`fm_origin=agent`; **default `user`**), and the acting client rides in `azp`. Persisted records store `owner=preferred_username`, `origin=fm_origin`, `actor=azp`, so UIs render "alice" or "alice (via agent)". No synthetic per-user users. The claim is stamped by the realm (`fm-origin` client scope = `user`; the `agents` client overrides to `agent`) and **fm_runtime propagates it across exchange hops** so downstream audiences see the true origin. The `agents` service (`:8006`, `/api/agents`) is **browser-facing** — gateway/nginx-routed like search/mail (the browser `frontend` client now also mints an `agents` audience, so `azp_allow.agents` lists `frontend` alongside `jobs`), backed by its own `agents-db` (db `funnelmanager_agents`); its only egress edges are `agents→mcp` (RFC 8693; the runtime agent acts **exclusively** through MCP tools) and the `agents→agents-db` TCP pairing. Its standalone SPA `agentsui` is served same-origin at `/agents/` (own container, `mailui` pattern) and appears on the hub as a `WEB_APPS` tile (`/agents/`, probe `/api/agents/whoami`).
- **Principle 4 — guard expensive actions.** Any operation that may consume extreme resources (a full mailbox backup, a large embeddings backfill, a huge search or campaign) must **estimate first and require confirmation before proceeding** — never silently run something costly. Pattern (a shared `fm_runtime` helper so every service does it identically): the endpoint computes an estimate; if it exceeds a **configurable threshold** it returns **`409 confirmation_required`** with the estimate + a `confirm` token instead of running; the caller re-invokes with `confirm=true`. A UI renders a dialog; a **runtime AI agent must escalate the confirmation to its human** (never auto-confirm). Concrete default: **mailbox backup > 2 GB** requires confirmation.
- **MCP is the agents' whole capability surface, and it can start work.** Runtime agents act **only** through MCP tools (one audited surface). MCP is refactored to a **modular, multi-audience** architecture (audience-parameterized token exchange; one `BackendClient` per upstream: leads/search/jobs/mail). Its tool set is comprehensive per service and includes **write** tools — notably `start_apollo_search` / `start_semantic_search` / `enrich_leads`: **MCP can start searches**, funneled through search → leads (the "only leads talks to Apollo" invariant still holds — MCP never calls Apollo directly). `jobs` tools (`list_jobs`/`get_job`/`pause`/`resume`/`cancel`) give agents situational awareness of running work.
- **v1 versioning (stability contract).** MCP-facing and internal routes are **versioned**: `/api/{service}/mcp/v1/*` (MCP endpoints, distinct from the UI's routes — principle 2), `/internal/{domain}/v1/*` (job-event stream + job control). Within a version changes are **additive only**; a breaking change ships as a **new version** (`v2`) alongside the old. **MCP tool names + schemas are a stable contract** to agents — evolve additively, never silently repurpose a tool.
- **Jobs contract.** Each app that produces jobs exposes an internal NDJSON stream `GET /internal/jobs/v1/stream` (audience `= app`, `jobs`-role grant) and control `POST /internal/jobs/v1/{id}/{pause|resume|cancel}` (only the `jobs` service audience may call). The `jobs` service subscribes (exchanging → the app's audience) and persists every running job. v1 producers are **`search` and `agents` only** (config-driven via `JOBS_PRODUCERS`, so `mail`/others join later); each agent run is itself a job (pausable/cancelable). Reaching a producer's `/internal/jobs/*` is the **machine-only `jobs-internal` realm role**, held by the `jobs` client's service account (`azp=jobs`, client-credentials — never a human; the acting human is audit metadata). Both `jobs` and `agents` are **wired into `docker-compose.{dev,prod}.yml` (dedicated `jobs-db`/`agents-db` Postgres → dbs `funnelmanager_jobs`/`funnelmanager_agents`, the `mail-db` pattern) and k3s (`deploy/apps/base/{jobs,agents}` + `.../data/{jobs-db,agents-db}` + netpol, both overlays)**; both producers are now configured (`JOBS_PRODUCERS=search=http://search:8000,agents=http://agents:8006`). The `jobs→search`/`jobs→agents` caller edges + the `jobs→jobs-db`/`agents→agents-db` TCP pairings are in `deploy/policy/data.json`; `jobs-internal` (scoped to `/internal/jobs` on `search`+`agents`) lives in `grants.py` ⇔ `data.json` ⇔ both realms (assigned to `service-account-jobs`), provable with `python -m fm_runtime.export --check … --realm`.

The MCP server (`:8003/mcp`) takes a **per-tool-call token** (the acting principal's, aud `mcp`): every tool accepts `session_token` (an `Authorization: Bearer` header on `/mcp` also works; the explicit argument wins — see `_token` in `mcp/app/main.py`) and exchanges it (RFC 8693) for the **target upstream's** audience per call — one `BackendClient` per backend it fans out to (`leads`/`search`/`jobs`/`mail`), each reached at its own internal URL (`LEADS_BACKEND_URL`, `SEARCH_BACKEND_URL=http://search:8000`, `JOBS_BACKEND_URL=http://jobs:8005`, mail). The one-hop exchange edges `mcp→search` / `mcp→jobs` / `mcp→mail` (svc-scopes in the realm, `azp_allow` in `data.json`, `SVC_EXCHANGE_SCOPES` in `grants.py`) authorize those hops. Tokenless calls only work when `MCP_SHARED_LOGIN_FALLBACK=true` (dev compose) — they act as the MCP server's own service identity via client credentials. Tools span read-only inspection over leads (`leads_stats`, `recent_leads`, `get_leads`, `similarity_search`, all `readOnlyHint`) and the write/awareness surface described above (search-started work funnels through search → leads, so only leads ever talks to Apollo; `jobs` tools observe running work). The transport has a Host-header allowlist (`MCP_ALLOWED_HOSTS`) — internal clients dial `mcp:8003`. Prod publishes it on loopback only (`127.0.0.1:8003`).
