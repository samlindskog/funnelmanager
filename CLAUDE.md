...
- **Profiles + roles live in Redis** (`auth/app/store.py`): hashes `users`, `roles`, `account_requests`. A user has a bcrypt hash and exactly one role; a role has grants `{service, methods, path_prefix}`. `AUTH_USERNAME`/`AUTH_PASSWORD` only **bootstrap** the admin user (created if missing, never updated — see `bootstrap()` in `auth/app/main.py`).
...
- **Auth's access tiers are per-route, not OPA**: `login`/`request-account` anonymous; `me`/`logout`/`apps`/`validate`/`authorize` any valid profile (the token in the body *is* the credential); everything else admin-gated via OPA.
