...
- **Principle 1 — uniform functionality / cross-user visibility (within a service, gated per service).** Access is gated **per service** by a distinct realm role: `search-access`, `mail-access`, `jobs-access`, `agents-access` (plus `admin` = all, `internal-service` = leads-only). A principal may call a service's API **only if it holds that service's access role** — otherwise OPA (mesh) / `fm_runtime` grants (compose) return 403. *Within* a service every principal that has access sees the *same* features and the *same* data; there is **no per-user data hiding in app code** — the realm role is the only access gate, and it gates *whether you may call the API*, not *which rows* you see. So search history, mail inboxes, and campaigns are visible to **every user with that service's access role** (not to users lacking it). Writes are still *attributed* to the acting principal (`username` owner) — just not *hidden*. New read APIs browse by any user or all together. These roles are encoded in three places that must stay in sync: `fm_runtime/grants.py` `_DEFAULT_ROLE_GRANTS`, `deploy/policy/data.json` `funnelmanager.roles`, and the Keycloak realm roles (`admin` is a composite of the four `-access` roles). `python -m fm_runtime.export --check deploy/policy/data.json` machine-proves the first two equal (and `--realm` catches svc-scope over-grants); the realm-role **definitions** are currently kept in sync by hand — extending `verify_policy` to assert them is a tracked follow-up.
