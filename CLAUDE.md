...
- **Dev compose has no mesh**, so backends verify JWTs locally (`FM_JWT_VERIFY=true` + JWKS). The issuer is pinned to the browser-facing URL (`http://localhost:8080/...`) while token/JWKS URLs dial the `keycloak` container — keep that split or `iss` validation breaks.
