...
- **Principle 1 — uniform functionality / cross-user visibility.** Every authenticated user of a service sees the *same* features and the *same* data; there is **no per-user data hiding in app code** — Keycloak (per-service audience + role) is the only access gate. This reverses the old owner-only filtering: search history, mail inboxes, and campaigns become visible to **every** user of that service. Writes are still *attributed* to the acting principal (`username` owner) — just not *hidden*. New read APIs browse by any user or all together.
